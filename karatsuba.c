...
char *split_at(char *x, int inicio, int fim){
    char *a=(char *)malloc(sizeof(char)*(fim-inicio+1));
    int i;
...
char *karatsuba(char *a, char *b, int k){
    ...
    char *p1=(char *)malloc(sizeof(char *)*(2*k));
	char *p2=(char *)malloc(sizeof(char *)*(2*k));
	char *p3=(char *)malloc(sizeof(char *)*(2*k));

	int m=n/2;

	char *a1=(char *)malloc(sizeof(char *)*(2*k));
	a1=split_at(ax,0, m);
	char *a2=(char *)malloc(sizeof(char *)*(2*k));
	a2=split_at(ax,m,n);
	char *b1=(char *)malloc(sizeof(char *)*(2*k));
	b1=split_at(bx,0, m);
	char *b2=(char *)malloc(sizeof(char *)*(2*k));
	b2=split_at(bx,m,n);
	free(ax);
	free(bx);
    p1=karatsuba(a1, b1, m);
    char *uh=(char *)malloc(sizeof(char *)*(2*k));
    strcpy(uh,p1);
	...
	for(i=0;i<n;i++)
		...
		kma[i]='\0';
	strcat(p1,kma);
	char *res=(char *)malloc(sizeof(char *)*2*k);
	res=combine(p1,p2);
	char *q=(char *)malloc(sizeof(char *)*2*k);
	q=combine(uh,p2);
	q=difference(p3,q);
	for(i=0;i<m;i++)
		kma[i]='0';
		kma[i]='\0';
	strcat(q,kma);
	char *sur=(char *)malloc(sizeof(char *)*2*k);
	sur=combine(q,res);
...
void printResposta(char *x, int n){
	int flag=0;
	int i=0;
	while(flag==0){
        if(x[i]=='0' && flag==0 && i<n){
...
int main(){
    ...
    printf("Insira o primeiro multiplicador: ");
    scanf(" %s", a);
    printf("Insira o segundo multiplicador: ");
    scanf(" %s", b);
    long k=strlen(a);
    long o=strlen(b);
    ...
    //número de digitos é igual? senao poe zero na parte mais significativa do menor
    if(k>o){
